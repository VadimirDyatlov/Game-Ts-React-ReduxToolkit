...
export function radianceNumbers(this: any, classN: string): void {
...
export function getLvl(num: number): number[] {
  ...
  if (lvlStack[0] <= num && num < lvlStack[1]) return [2, lvlStack[1]];
  if (lvlStack[1] <= num && num < lvlStack[2]) return [3, lvlStack[2]];
  if (lvlStack[2] <= num && num < lvlStack[3]) return [4, lvlStack[3]];
  if (lvlStack[3] <= num && num < lvlStack[4]) return [5, lvlStack[4]];
  if (lvlStack[4] <= num && num < lvlStack[5]) return [6, lvlStack[5]];
  if (lvlStack[5] <= num && num < lvlStack[6]) return [7, lvlStack[6]];
  if (lvlStack[6] <= num && num < lvlStack[7]) return [8, lvlStack[7]];
  if (lvlStack[8] <= num && num < lvlStack[9]) return [9, lvlStack[8]];
  if (num > lvlStack[9]) return [10, lvlStack[9]];
  return [0, 0];
}
...
export function objectsCollision(first: IHero | IBaullet, second: IEnemies | IGold): boolean {
